...
class FilterButtonView: UIView {
    ...
    func showFilterButtons() {
        for index in 2...numberOfButtons {
            UIView.animateWithDuration(0.5, delay: NSTimeInterval(Double(index - 1) * 0.05), usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: [], animations: {
                NSLog(".") //༼;´༎ຶ ۝ ༎ຶ༽
    ...
    func hideFilterButtons() {
        positionView()
        for index in 2...numberOfButtons {
            UIView.animateWithDuration(0.18, delay: 0.05, options: [], animations: {
    ...
    func buttonAction(sender: UIButton!) {
        ...
        if(self.opened != true){
            ...
            currentBtn.setImage(UIImage(named: (btnImageName as String)+"0"), forState: UIControlState.Normal)
            currentBtn.setImage(UIImage(named: (btnImageName as String)+"1"), forState: UIControlState.Highlighted)
